...
已知DLP-M具备多级层次存储结构，对于MLU-Core而言，不同层级的存储空间在I/O效率和可见性方面存在差异。在完成数据拆分后，有必要进一步对各变量在不同存储上的空间进行合理划分。
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{\textwidth}
        \centering
        \includegraphics[scale = 0.6]{nram_split_lk.png}
    \caption{NRAM内存划分}
...
当 \( B\_split \) 数据量超出 NRAM 存储空间容量时，需将 \( B\_split \) 进一步划分为若干段，通过循环机制依次将数据加载至 NRAM 空间进行处理。从硬件层面分析，上述数据拆分过程的核心在于明确各 MLU - Core 的输入数据计算范畴。就具体实现流程而言，首先需考量输入数据及其他中间变量在各层级存储中的空间分配策略。  
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{\textwidth}
        \centering
        \includegraphics[scale = 0.55]{gdram_split_bk.png}
        \caption{GDRAM空间拆分示意图}
        \label{fig:gdram_split_bk}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{\textwidth}
        \centering
        \includegraphics[scale = 0.56]{workspace_split_bk.png}
    \caption{Workspace空间拆分示意图}
    \label{fig:workspace_split_bk}
    \end{subfigure}
    ...
    \begin{subfigure}[c]{\textwidth}
        \centering
        \includegraphics[scale = 0.55]{nram_split_bk.png}
    \caption{NRAM空间拆分示意图}
...
\begin{figure}[ht]
    \centering
    \includegraphics[scale = 0.75]{data0_int32.png}
    \caption{整型输入数据示意图}
...
\begin{figure}[ht]
    \centering
    \includegraphics[scale = 0.75]{data1_int32.png}
...
\begin{figure}[ht]
    \centering
    \includegraphics[scale = 0.75]{res_int32.png}
    \caption{整型正确输出示意图}
    \label{fig:res_int32}
    % \note{注：图注的内容不宜放到图题中。}
\end{figure}
...
\begin{figure}[ht]
    \centering
    \includegraphics[scale = 0.75]{data0_float32.png}
    \caption{浮点型输入数据示意图}
    \label{fig:data0}
    % \note{注：图注的内容不宜放到图题中。}
\end{figure}
...
\begin{figure}[ht]
    \centering
    \includegraphics[scale = 0.75]{data1_float32.png}
...
\begin{figure}[ht]
    \centering
    \includegraphics[scale = 0.75]{res_float32.png}
    \caption{浮点型正确输出示意图}
