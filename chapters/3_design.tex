...
2.求取L和R时，意在求得当前input中小于p的部分放于L，大于p的部分放于R，
...
利用 \text{bang\_lt\_scalar} 和 \text{bang\_lt\_not} 向量化指令处理其局部数据，生成局部 \(B\)（\(B\_\text{local}\)）。
利用 \text{bang\_count\_mask} 计算出 \(B\_\text{local}\) 中的有效的个数 \(\text{count\_local}\)，其结果决定了多处理器的结果在片外内存
上的 \(\text{offset}\)，计算完成后将其放回到片外内存的相应位置，供各处理器使用。

2、对整个数组进行第二次遍历，保证与第一次遍历时的数据拆分相同，
再次利用 \text{bang\_lt\_scalar} 和 \text{bang\_lt\_not} 向量化指令生成局部 \(B\)（\(B\_\text{local}\)），
而后使用 \text{bang\_count\_mask} 计算出当前有效数据的个数，其结果作为处理器内循环在片外显存的 \(\text{offset}\)。
